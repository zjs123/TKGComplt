...
class ILP:
    ...
    def __init__(self,args):
        ...
        #self.ILP_solver()
        if args.Type == "r":
            self.ILP_R_stat(args.testHead, args.testRelation, args.testTail,args.testTime, args.Type)
        else :
            self.ILP_E_stat(args.testHead, args.testRelation, args.testTail, args.testTime,args.Type)
    ...
    def ILP_entity(self):
        ...
        for i in range(self.numOfTestTriple):
            ...
            if C_1*C_2 !=1 or float(targetLoss[0]) > 11:
                targetLoss=torch.FloatTensor([1000]).repeat(self.numOfEntity,1)
            ...
            for j in range(self.numOfEntity):
                ...
                if C_1_h*C_2_h !=1 or float(tmpHeadLoss[j]) > 12:
                    ILP_list_h.append([10000])
                else:
                    ILP_list_h.append([0])
                if C_1_t*C_2_t !=1 or float(tmpTailLoss[j]) > 12:
    ...
    def ILP_relation(self):
        ...
        for i in range(self.numOfTestTriple):
            ...
            if C_1*C_2_pre*C_2_aft ==0 or (float(targetLoss_trans[0])) > 11:
                targetLoss_trans=torch.FloatTensor([1000]).repeat(self.numOfRelation,1)
            
            tmpRelationLoss=torch.norm(tmpHeadEmbedding+self.Transmit.relation_embeddings.weight.data-tmpTailEmbedding,self.norm,1).view(-1,1)#10*1
            ...
            for j in range(self.numOfRelation):
                ...
                if C_1*C_2_pre*C_2_aft ==0 or (float(tmpRelationLoss[j])) > 11:
            ...
            wrongRelation = torch.nonzero(nn.functional.relu(targetLoss_trans-tmpRelationLoss))
            rankR=torch.nonzero(nn.functional.relu(targetLoss_trans-tmpRelationLoss)).size()[0]+1
    ...
    def ILP_E_stat(self,head,relation,tail,time,Type):
        head_embedding=self.Transmit.entity_embeddings(torch.tensor([head]))
        relation_embedding=self.Transmit.relation_embeddings(torch.tensor([relation]))
        tail_embedding=self.Transmit.entity_embeddings(torch.tensor([tail]))
        
        target_loss = torch.norm(head_embedding+relation_embedding-tail_embedding,self.norm).repeat(self.numOfEntity,1)
    
        C_1=self.overlap_constrain_h(head,relation,tail,time)
        C_2=self.order_constrain_h(head,relation,tail,time)
        if C_1*C_2 !=1 or float(target_loss[0]) > 11:
            target_loss=torch.FloatTensor([1000]).repeat(self.numOfEntity,1)
            
        head_embedding = head_embedding.repeat(self.numOfEntity,1)
        relation_embedding = relation_embedding.repeat(self.numOfEntity,1)
        tail_embedding = tail_embedding.repeat(self.numOfEntity,1)
        
        tmpHeadLoss=torch.norm(self.Transmit.entity_embeddings.weight.data + relation_embedding - tail_embedding,
                                 self.norm, 1).view(-1, 1)
        tmpTailLoss = torch.norm(head_embedding + relation_embedding - self.Transmit.entity_embeddings.weight.data,
                                 self.norm, 1).view(-1, 1)
        
        ILP_list_h=list()
        ILP_list_t=list()
        for j in range(self.numOfEntity):
            C_1_h=self.overlap_constrain_h(int(j),relation,tail,time)
            C_2_h=self.order_constrain_h(int(j),relation,tail,time)
            C_1_t=self.overlap_constrain_t(head,relation,int(j),time)
            C_2_t=self.order_constrain_t(head,relation,int(j),time)
        ...
        tmpHeadLoss=tmpHeadLoss+ILP_tensor_h
        tmpTailLoss=tmpTailLoss+ILP_tensor_t
        wrongHead=torch.nonzero(nn.functional.relu(target_loss - tmpHeadLoss))
        wrongTail=torch.nonzero(nn.functional.relu(target_loss - tmpTailLoss))
        
        if Type == "h":
            print(wrongHead.size()[0])
            for tmp in wrongHead:
                print(tmp[0])
        
        if Type == "t":
            print(wrongTail.size()[0])
            for tmp in wrongTail:
                print(tmp[0])
    ...
    def ILP_R_stat(self,head,relation,tail,time,Type):
        ...
        target_loss = torch.norm(head_embedding+relation_embedding-tail_embedding,self.norm).repeat(self.numOfRelation,1)
        C_1=self.overlap_constrain(head,relation,tail,time.numpy())
        C_2_pre=self.order_constrain(head,relation,tail,time.numpy(),"pre")
        C_2_aft=self.order_constrain(head,relation,tail,time.numpy(),"aft")
        
        if C_1*C_2_pre*C_2_aft ==0 or (float(target_loss[0])) > 11:
            target_loss=torch.FloatTensor([1000]).repeat(self.numOfRelation,1)
        head_embedding = head_embedding.repeat(self.numOfRelation,1)
        tail_embedding = tail_embedding.repeat(self.numOfRelation,1)
        
        tmpRelationLoss=torch.norm(head_embedding+self.Transmit.relation_embeddings.weight.data-tail_embedding,self.norm,1).view(-1,1)
        ILP_list_r=list()
                
        for j in range(self.numOfRelation):
        
            C_1=self.overlap_constrain(head,int(j),tail,time.numpy())
            C_2_pre=self.order_constrain(head,int(j),tail,time.numpy(),"pre")
            C_2_aft=self.order_constrain(head,int(j),tail,time.numpy(),"aft")
        ...
        wrongRelation = torch.nonzero(nn.functional.relu(target_loss-tmpRelationLoss))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--testHead",dest="testHead",type=int,default=5882)
    parser.add_argument("--testRelation",dest="testRelation",type=int,default=7)
    parser.add_argument("--testTail",dest="testTail",type=int,default=9)
    parser.add_argument("--testTime",dest="testTime",type=list,default=[1959,1959])
    parser.add_argument("--Type",dest="Type",type=str,default="t")

...
class Train:
    ...
    def __init__(self,args):
        ...
        #self.ILP=ILP(self.Transmit,dataset='YAGO',numOfentity=10623,numOfRelation=10,numOfTestTriple=2051,norm=self.norm)
    ...
    def train(self):
        ...
        readData(path, self.train2id, self.seq_withTime, self.seq_relation,self.headRelation2Tail,self.tailRelation2Head,
        ...
        for epoch in range(self.numOfEpochs):
            ...
            for batch in dataLoader:
                ...
                positiveLoss,negativeLoss,positive_relation_pair_Loss,negative_relation_pair_Loss =self.Transmit(positiveBatchHead, positiveBatchRelation, positiveBatchTail,corruptedBatchHead,
                                   corruptedBatchRelation, corruptedBatchTail,relation_pair_h,relation_pair_t)
                ...
                normloss=Loss.normLoss(ent_embeddings)+Loss.normLoss(rel_embeddings)+0.1*Loss.orthogonalLoss(rel_embeddings,torch.mm(self.Transmit.relation_trans,torch.from_numpy(np.linalg.pinv(self.Transmit.relation_trans.numpy()))))
    ...
    def validate_relation(self):
        meanRank = 0
        for tmpTriple in range(self.numOfValidateTriple):
            meanRank += self.Transmit.fastValidate_relation(self.validateHead[tmpTriple], self.validateRelation[tmpTriple], self.validateTail[tmpTriple],self.validateTime[tmpTriple],self.seq_withTime)
        return meanRank/self.numOfValidateTriple
...
if __name__ == '__main__':
    ...
    parser.add_argument("--nepoch",dest="numOfEpochs",type=int,default=200)
    ...
    parser.add_argument("--tf",dest="tf",type=float,default=0.01)
